...
class PlotterClass:
    ...
    def plot_probs(self):
        ...
        probs = self.probs
        bers = self.BERs
        prob_highlight = [0.01, 0.020, 0.05, 0.1, 0.15]
        ...
        ax.scatter(prob_highlight, ber_highlight_A, color=colors[0], lw=self.lw, s=5, marker='*')
        ax.scatter(prob_highlight, ber_highlight_B, color=colors[1], lw=self.lw, s=5, marker='*')
        ...
        plt.xticks(np.arange(0,0.151, 0.01))
